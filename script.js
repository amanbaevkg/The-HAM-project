/*---------TABS в секции  <section class="container tabs"------>*/
const tabTitle = document.querySelectorAll(".tabs-title-item");
const lengthTabTitile = tabTitle.length;
const tabSwitcher = ()=>{
    return function (){
        let tabContent = document.querySelector(`.tabs-content-item[data-content="${this.dataset.content}"]`);
        document.querySelector('.tabs-title-item.active').classList.remove('active');
...
/*--------Секция <section class="portfolio">. Код отфильтровывает фотографии по категориям--------*/
...
morePhoto.onclick =()=>{
    wrapCard.forEach(e => { e.removeAttribute('data-load')});
    tabsFoto.forEach((el)=> {
